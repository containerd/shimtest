...
| `InboundTCPListen` | net | A container's init process binds a TCP listener on an ephemeral port, the shim maps that port to the host, and the host-side test connects and completes a round trip. Verifies the inbound (listen/accept) side of the default networking contract: ports bound inside the container must be reachable from the host. |

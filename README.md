...
| `InboundTCPListen` | net | A container's init process binds a TCP listener and the host-side test connects to it and confirms the container actually received the data. Without `provide_network`, the container binds an ephemeral port that the shim is assumed to map to the host; with `provide_network`, the test registers its own port forward before the container even starts. Verifies the inbound (listen/accept) side of the default networking contract: ports bound inside the container must be reachable from the host. |
